...
import { getOmnipairErrorInfo } from '@/protocol/omnipair'
...
function parseCustomProgramCodeFromMessage(message: string): number | null {
  const omnipairMatch = /omnipair\s*#\s*(\d+)/i.exec(message)
  if (omnipairMatch) return toNumberLike(omnipairMatch[1])
...
export function getSimulationCustomErrorCode(err: unknown): number | null {
  const normalized = extractSimulationError(err) ?? err
  const parsed = parseInstructionError(normalized)
  if (parsed?.kind === 'custom' && parsed.customCode !== null) {
  ...
  if (err instanceof Error) {
    return parseCustomProgramCodeFromMessage(err.message)
  }

  if (typeof err === 'string') {
    return parseCustomProgramCodeFromMessage(err)
  }

  if (err && typeof err === 'object') {
    const record = err as Record<string, unknown>
    if (typeof record.message === 'string') {
      const fromMessage = parseCustomProgramCodeFromMessage(record.message)
      if (fromMessage !== null) return fromMessage
    }
  }

  const json = safeJsonStringify(normalized)
  return parseCustomProgramCodeFromMessage(json)
}
