    ...
    !isWalletConnected && !hasPreviewInput
      ...
      : utilization < 0.5
        ...
        : utilization < 0.75
          ...
          ? 'High'
          : 'Critical'
  const borrowActionDisabled =
    borrowSubmitting ||
    (isWalletConnected &&
      (!borrowToken || !borrowPool || selectedBorrowPool?.statusLabel === 'Reduce-only'))
  ...
  return (
    <div className="borrow-shell borrow-center">
      <section className="trade-card borrow-card">
        ...
        >
          {borrowSubmitting ? 'Submitting…' : !isWalletConnected ? 'Connect First' : 'Borrow'}
        </button>
      </section>
    </div>
  )
