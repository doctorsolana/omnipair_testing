  ...
  const send = useCallback(
    async (instructions: Instruction<Address>[]) => {
      ...
      if (simResult?.value?.err) {
        throw new Error(`Simulation failed: ${formatSimulationError(simResult.value.err)}`)
      }

      let computeLimit = DEFAULT_COMPUTE_UNITS
      if (simResult?.value?.unitsConsumed) {
        ...
        if (Number.isFinite(unitsConsumed) && unitsConsumed > 0) {
          computeLimit = Math.min(
            MAX_COMPUTE_UNITS,
            Math.max(DEFAULT_COMPUTE_UNITS, Math.ceil(unitsConsumed * COMPUTE_UNIT_BUFFER)),
          )
        }
      }
